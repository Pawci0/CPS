    ...
    public class AcModel
    {
        public AcModel(RealSignal signal, double samplingFrequency, int numberOfLevels, int numberOfIncludedSamples)
        {
            ...
            UniformSampling = new ConvertedSignalModel()
            {
                ...
                PeakSignalToNoiseRatio = AnalogAndDigitalConverter.PeakSignalToNoiseRatio(OriginalSignal, convertedSignal),
            ...
            convertedSignal = AnalogAndDigitalConverter.UniformQuantizationWithTruncation(UniformSampling.Signal, numberOfLevels);
            ...
            {
                Signal = convertedSignal,
                MeanSquaredError = AnalogAndDigitalConverter.MeanSquaredError(UniformQuantizationWithTruncation.Signal, convertedSignal),
                SignalToNoiseRatio = AnalogAndDigitalConverter.SignalToNoiseRatio(UniformQuantizationWithTruncation.Signal, convertedSignal),
                PeakSignalToNoiseRatio = AnalogAndDigitalConverter.PeakSignalToNoiseRatio(UniformQuantizationWithTruncation.Signal, convertedSignal),
                MaximumDifference = AnalogAndDigitalConverter.MaximumDifference(UniformQuantizationWithTruncation.Signal, convertedSignal),
                EffectiveNumberOfBits = AnalogAndDigitalConverter.EffectiveNumberOfBits(UniformQuantizationWithTruncation.Signal, convertedSignal)
            };

            convertedSignal = AnalogAndDigitalConverter.ReconstructionBasedOnTheSincFunction(UniformSampling.Signal, numberOfIncludedSamples);

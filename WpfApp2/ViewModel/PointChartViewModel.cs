...
{
    public class PointChartViewModel : BindableObject
    {
        ...
        public void GenerateChart(RealSignal signal)
        {
            ...
            LineSeries.Add(new XyScatterRenderableSeriesViewModel
            {
                StrokeThickness = 2,
                PointMarker = new EllipsePointMarker() { Fill = Colors.SteelBlue },
                DataSeries = lineData
            });

            var histogramData = new XyDataSeries<double, double>() { SeriesName = HistogramTitle };

            foreach (var (x, y) in signal.ToDrawHistogram((int)signal.Length))
            {
